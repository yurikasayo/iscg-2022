...
{
    const dist = v.lengthSq() - 5 * 5
    return dist
}

const idToVector = (x, y, z, id) => {
    const v = new THREE.Vector3(
        x + Math.floor(id / 2) % 2,
        y + Math.floor((id+1) / 2) % 2,
        z + Math.floor(id / 4) % 2
    )
    return v
} 
...
const marchingTetrahedra = (isoValue) =>
{
    ...
    for (let x = -5; x < 5; x++) 
    {
        for (let y = -5; y < 5; y++) 
        {
            for (let z = -10; z < 5; z++) 
            {
                ...
                {
                    const v0 = idToVector(x, y, z, tetraPositions[i][0])
                    const v1 = idToVector(x, y, z, tetraPositions[i][1])
                    const v2 = idToVector(x, y, z, tetraPositions[i][2])
                    const v3 = idToVector(x, y, z, tetraPositions[i][3])
                    ...
                    switch(triIndex)
                    {
                        ...
                        case 9:
                            faceNum+=2
                            faceids.push(vertices.length, vertices.length + 1, vertices.length + 2, vertices.length, vertices.length + 3, vertices.length + 2)
                            vertices.push(interpolate(isoValue, v0, v1, f0, f1))
                            vertices.push(interpolate(isoValue, v1, v3, f1, f3))
                            vertices.push(interpolate(isoValue, v2, v3, f2, f3))
                            vertices.push(interpolate(isoValue, v0, v2, f0, f2))
...
const geometry = marchingTetrahedra(0.00001)
